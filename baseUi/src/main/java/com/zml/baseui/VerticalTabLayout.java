...
public class VerticalTabLayout extends ScrollView {
    ...
    public interface OnTabSelectedListener {
        ...
        void onTabSelected(VerticalTab tab);

        void onTabUnselected(VerticalTab tab);

        void onTabReselected(VerticalTab tab);
    }
    ...
    /**
     * configure for {@link #setupWithViewPager(ViewPager2)}
     */
    ...
    private void initStyleConfigure(Context context, AttributeSet attrs) {
        ...
        mColorIndicator = typedArray.getColor(R.styleable.VerticalTabLayout_indicator_color, Color.BLACK);
    ...
    public VerticalTab newTab() {
        VerticalTab tab = sTabPool.acquire();
        if (tab == null) {
            tab = new VerticalTab();
        }
        tab.mParent = this;
    ...
    public void addTab(VerticalTab tab) {
        addTab(tab, tabs.size());
    }

    public void addTab(VerticalTab tab, int position) {
        addTab(tab, position, false);
    }

    public void addTab(VerticalTab tab, boolean selected) {
        addTab(tab, tabs.size(), selected);
    }

    public void addTab(VerticalTab tab, int position, boolean selected) {
    ...
    private TabView createTabView(@NonNull final VerticalTab tab) {
    ...
    private void configureTab(VerticalTab tab, int position) {
    ...
    private void addTabView(VerticalTab tab) {
    ...
    public void removeTabAt(int position) {
        ...
        final VerticalTab removedTab = tabs.remove(position);
        if (removedTab != null) {
            removedTab.reset();
    ...
    public void removeAllTabs() {
        ...
        for (final Iterator<VerticalTab> i = tabs.iterator(); i.hasNext(); ) {
            final VerticalTab tab = i.next();
            i.remove();
            tab.reset();
    ...
    public VerticalTab getTabAt(int index) {
        return (index < 0 || index >= getTabCount()) ? null : tabs.get(index);
    }

    void selectTab(final VerticalTab tab) {
        selectTab(tab, true);
    }

    void selectTab(final VerticalTab tab, boolean updateIndicator) {
        final VerticalTab currentTab = mSelectedTab;
        if (currentTab == tab) {
            if (currentTab != null) {
    ...
    private void dispatchTabSelected(VerticalTab tab) {
        for (int i = 0; i < mTabSelectedListeners.size(); i++) {
            mTabSelectedListeners.get(i).onTabSelected(tab);
        }
    }

    private void dispatchTabUnselected(VerticalTab tab) {
        for (int i = 0; i < mTabSelectedListeners.size(); i++) {
            mTabSelectedListeners.get(i).onTabUnselected(tab);
        }
    }

    private void dispatchTabReselected(VerticalTab tab) {
        for (int i = 0; i < mTabSelectedListeners.size(); i++) {
    ...
    public void setupWithViewPager(ViewPager2 viewPager, ViewPagerTabItemCreator creator) {
    ...
    private void setPagerAdapter(@Nullable final RecyclerView.Adapter adapter) {
    ...
    public static final class VerticalTab {
        ...
        @NonNull
        public VerticalTab setTag(@Nullable Object tag) {
            mTag = tag;
            return this;
        }
        ...
        @NonNull
        public VerticalTab setCustomView(@Nullable View view, OnCustomTabViewRenderListener listener) {
            mCustomView = view;
        ...
        @NonNull
        public VerticalTab setCustomView(@LayoutRes int resId, OnCustomTabViewRenderListener listener) {
        ...
        @NonNull
        public VerticalTab setIcon(@Nullable Drawable icon) {
            mIcon = icon;
            updateView();
            return this;
        }
        ...
        @NonNull
        public VerticalTab setIcon(@DrawableRes int resId) {
            if (mParent == null) {
        ...
        @NonNull
        public VerticalTab setText(@Nullable CharSequence text) {
            mText = text;
            updateView();
            return this;
        }
        ...
        @NonNull
        public VerticalTab setText(@StringRes int resId) {
            if (mParent == null) {
                throw new IllegalArgumentException("Tab not attached to a TabLayout");
            }
        ...
        @NonNull
        public VerticalTab setContentDescription(@StringRes int resId) {
            if (mParent == null) {
                throw new IllegalArgumentException("Tab not attached to a TabLayout");
            }
        ...
        @NonNull
        public VerticalTab setContentDescription(@Nullable CharSequence contentDesc) {
     ...
     * 如果需要复杂的视图效果可以通过{@link VerticalTab#setCustomView(View, OnCustomTabViewRenderListener)}设置自定义的view
     */
    class TabView extends LinearLayout {
        ...
        void setTab(@Nullable final VerticalTab tab) {
            if (mTab != tab) {
                mTab = tab;
                update();
            }
        ...
        final void update() {
            ...
            if (mCustomView == null) {
                ...
                if (mIconView == null) {
                    ...
                    ImageView iconView = (ImageView) LayoutInflater.from(getContext()).inflate(
                            com.google.android.material.R.layout.design_layout_tab_icon, this, false);
                    addView(iconView, 0);
                    mIconView = iconView;
                }
                if (mTextView == null) {
    ...
    private static class OnTabPageChangeListener extends ViewPager2.OnPageChangeCallback {
    ...
    public static class ViewPagerOnVerticalTabSelectedListener implements OnTabSelectedListener {
        ...
        public void onTabSelected(VerticalTab tab) {
            ...
            if (viewPager != null && viewPager.getAdapter().getItemCount() >= tab.getPosition()) {
                viewPager.setCurrentItem(tab.getPosition());
            }
        }

        @Override
        public void onTabUnselected(VerticalTab tab) {

        }

        @Override
        public void onTabReselected(VerticalTab tab) {
