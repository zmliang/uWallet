...
fun NavController.jumpByLogin(action: (NavController) -> Unit) {
    if (CacheUtil.isLogin()) {
        ...
        //this.navigate(R.id.action_to_loginFragment) todo
    }
}
...
fun <T> loadListData(
    ...
    if (data.isSuccess) {
        ...
        if (data.isRefresh) {
            ...
            if (data.errMessage.isNotEmpty()) {
                loadService.setCallBack(ErrorCallback::class.java) { _, view ->
                    view.findViewById<TextView>(R.id.error_text).text = data.errMessage
                }
            }
        } else {
